...
BleManager:   â†’ SYSTOLIC = 111
BleManager:   â†’ DIASTOLIC = 65
BleManager:   â†’ PULSE = 75
...
2. **Map semantic keys to UIDs**:
   ```kotlin
   readings.forEach { (key, value) ->
       val measurementKey = key.lowercase()  // "SYSTOLIC" â†’ "systolic"
       val fieldUid = sensorConfig.measurements?.get(measurementKey)?.dataElement
   }
   ```
...
4. **Save to correct fields**:
   - `SYSTOLIC=111` â†’ `HkfzcXMdLLF` (systolic field)
   - `DIASTOLIC=65` â†’ `skBarAsIYIL` (diastolic field)
   - `PULSE=75` â†’ `tZbUrUbhUNy` (pulse field)
...
â”‚ Systolic BP:    [111] mmHg          â”‚   Correct value
â”‚ Diastolic BP:   [65]  mmHg          â”‚   Correct value
â”‚ Pulse Rate:     [75]  bpm           â”‚   Correct value
...
**Before**: `readings[0]` â†’ what does this mean?  
**After**: `readings["SYSTOLIC"]` â†’ clear meaning
...
```json
{
  "measurements": {
    ...
    "diastolic": {"dataElement": "skBarAsIYIL", "unit": "mmHg"},
    "pulse": {"dataElement": "tZbUrUbhUNy", "unit": "bpm"},
    "map": {"dataElement": "NEW_MAP_UID", "unit": "mmHg"}  â† Just add to config
  }
}
```
...
1. **Update sensor configuration** in DHIS2 datastore:
   ...
   {
     "name": "Blood Pressure",
     "type": "multi",  â† Add this
     "measurements": {  â† Add this
       "systolic": {"dataElement": "HkfzcXMdLLF", "unit": "mmHg"},
       "diastolic": {"dataElement": "skBarAsIYIL", "unit": "mmHg"},
       "pulse": {"dataElement": "tZbUrUbhUNy", "unit": "bpm"}
     }
