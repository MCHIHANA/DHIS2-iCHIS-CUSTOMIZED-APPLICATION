...
â”‚   â”œâ”€â”€ androidUnitTest/kotlin/# Android unit tests
â”‚   â”œâ”€â”€ desktopMain/kotlin/    # Desktop-specific code
â”‚   â””â”€â”€ iosMain/kotlin/        # iOS-specific code (when applicable)
```
...
- **Mocking**: Use Mockito (`org.mockito.kotlin`) for mocking â€” `mock()`, `whenever()`, `doReturn`, `verify()`
...
- **Pattern**: Instantiate real use cases with a mocked repository; inject a `StandardTestDispatcher`; observe the ViewModel's `StateFlow` via [Turbine](https://github.com/cashapp/turbine) (`flow.test { â€¦ }`)
- **Mocking**: Use Mockito (`org.mockito.kotlin`) â€” `mock()`, `whenever()`, `doReturn`, `doReturnConsecutively`
- **Async handling**: Use `StandardTestDispatcher` + `Dispatchers.setMain(testDispatcher)` / `Dispatchers.resetMain()` and `runTest { â€¦ }`
- **Best practices**:
