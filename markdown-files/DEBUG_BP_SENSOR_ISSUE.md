...
readings.forEachIndexed { index, (key, value) ->
    val fieldUid = when (index) {
        0 -> primaryUid  // â† Only this gets saved!
...
This means:
- Index 0 (SYSTOLIC) â†’ saved to whichever field you tapped
- Index 1 (DIASTOLIC) â†’ skipped (secondarySensorFieldUid is null)
- Index 2 (PULSE) â†’ skipped (else branch)
...
1. **Clear app data**: Settings â†’ Apps â†’ DHIS2 â†’ Clear Data
2. **Open app** and login
3. **Sync metadata**: Settings â†’ Sync metadata
...
3. **Login**
4. **Sync metadata** (Settings â†’ Sync)
5. **Test BP sensor**
...
BleManager:   â†’ SYSTOLIC = 120
BleManager:   â†’ DIASTOLIC = 80
...
3. **All 3 values** should be saved automatically:
   - Systolic â†’ HkfzcXMdLLF
   - Diastolic â†’ BaGxiB8AsNI
   - Pulse â†’ S7OjKl85YSh
