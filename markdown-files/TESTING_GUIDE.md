...
**Expected Logcat Output**:
```
BLE_BP: Blood pressure values out of range â€” skipping
```
...
// Mantissa = 1200, Exponent = -1 â†’ 1200 Ã— 10^-1 = 120.0
```
...
fun `parseSFloat should handle negative exponent`() {
    val data = byteArrayOf(0x00, 0xB8.toByte(), 0xF0.toByte()) // 1200 Ã— 10^-1
    val result = BleDataParser.parseSFloat(data, 1)
    assertEquals(120.0f, result, 0.01f)
}
...
fun `parseBloodPressure should convert kPa to mmHg`() {
    ...
    val result = BleDataParser.parseBloodPressure(data)
    assertEquals(120.0f, result.systolic, 1.0f)  // 16 Ã— 7.50062 â‰ˆ 120
    assertEquals(82.5f, result.diastolic, 1.0f)  // 11 Ã— 7.50062 â‰ˆ 82.5
}
```
...
2. Restart app
3. Clear Bluetooth cache (Settings â†’ Apps â†’ Bluetooth â†’ Clear Cache)
