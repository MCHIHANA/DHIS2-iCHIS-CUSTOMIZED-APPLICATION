...
fun typeFor(mac: String): SensorType = when (mac) {
    ...
    GLUCOSE_SENSOR -> SensorType.GLUCOSE  // ← New
...
override fun onServicesDiscovered(gatt: BluetoothGatt, status: Int) {
    when (currentSensorType) {
        SensorType.GLUCOSE -> subscribeGlucose(gatt)  // ← New
        // ... other types
    }
}
...
private fun dispatchReading(uuid: String, data: ByteArray) {
    when (currentSensorType) {
        SensorType.GLUCOSE -> handleGlucoseData(uuid, data)  // ← New
        // ... other types
    }
}
